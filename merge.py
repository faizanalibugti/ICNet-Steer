import tensorflow as tf
import scipy.misc
import cv2
import mss
import time
import numpy as np

# ICNet
from tqdm import trange
from utils.config import Config
from model import ICNet, ICNet_BN

# Steering
from nets.pilotNet import PilotNet

# ICNet Model
model_config = {'train': ICNet, 'trainval': ICNet, 'train_bn': ICNet_BN, 'trainval_bn': ICNet_BN, 'others': ICNet_BN}

# Choose dataset here, but remember to use `script/downlaod_weight.py` first
dataset = 'cityscapes'
filter_scale = 1
    
class InferenceConfig(Config):
    def __init__(self, dataset, is_training, filter_scale):
        Config.__init__(self, dataset, is_training, filter_scale)
    
    # You can choose different model here, see "model_config" dictionary. If you choose "others", 
    # it means that you use self-trained model, you need to change "filter_scale" to 2.
    model_type = 'trainval'

    # Set pre-trained weights here (You can download weight from Google Drive) 
    model_weight = './model/cityscapes/icnet_cityscapes_trainval_90k.npy'
    
    # Define default input size here
    INFER_SIZE = (256, 512, 3)

cfg = InferenceConfig(dataset, is_training=False, filter_scale=filter_scale)
cfg.display()

# Create graph here 
model = model_config[cfg.model_type]
net = model(cfg=cfg, mode='inference')

# Create session & restore weight!
net.create_session()
net.restore(cfg.model_weight)

# Steering Angle
FLAGS = tf.app.flags.FLAGS

"""model from nvidia's training"""
tf.app.flags.DEFINE_string(
    'model', './save/model_nvidia.ckpt',
    """Path to the model parameter file.""")

tf.app.flags.DEFINE_string(
    'steer_image', './steering_wheel_image.jpg',
    """Steering wheel image to show corresponding steering wheel angle.""")

font                   = cv2.FONT_HERSHEY_SIMPLEX
bottomLeftCornerOfText = (0,50)
fontScale              = 1
fontColor              = (255,255,255)
lineType               = 2

if __name__ == '__main__':
    img = cv2.imread(FLAGS.steer_image, 0)
    rows, cols = img.shape

    cap = cv2.VideoCapture(0)

    with tf.Graph().as_default():
        smoothed_angle = 0
        i = 0

        # construct model
        model = PilotNet()

        saver = tf.train.Saver()

        with tf.Session() as sess:
            # restore model variables
            saver.restore(sess, FLAGS.model)

            with mss.mss() as sct:
                # Part of the screen to capture
                monitor = {"top": 0, "left": 0, "width": 640, "height": 200}

                while "Screen capturing":
                    last_time = time.time()

                    # Get raw pixels from the screen, save it to a Numpy array
                    screen = np.array(sct.grab(monitor))
                    screen = np.flip(screen[:, :, :3], 2) # BGR image input to neural networks
                    capture = cv2.cvtColor(screen, cv2.COLOR_BGR2RGB)

                    # Steering angle prediction
                    image = scipy.misc.imresize(screen, [66, 200]) / 255.0
                    # image = cv2.resize(screen, (200, 66)) / 255.0

                    steering = sess.run(
                        model.steering,
                        feed_dict={
                            model.image_input: [image],
                            model.keep_prob: 1.0
                        }
                    )

                    degrees = steering[0][0] * 180.0 / scipy.pi

                    print("Steering angle: " + str(degrees) + " degrees")

                    # Steering wheel visulization
                    smoothed_angle += 0.2 * pow(abs((degrees - smoothed_angle)), 2.0 / 3.0) * (
                            degrees - smoothed_angle) / abs(degrees - smoothed_angle)
                    M = cv2.getRotationMatrix2D(
                        (cols/2, rows/2), -smoothed_angle, 1)
                    dst = cv2.warpAffine(img, M, (cols, rows))
                    cv2.imshow("Steering Wheel", dst)

                    i += 1

                    # ICNet inference
                    segment = cv2.resize(screen, (cfg.INFER_SIZE[1], cfg.INFER_SIZE[0]))
                    # segment = scipy.misc.imresize(screen, (cfg.INFER_SIZE[0], cfg.INFER_SIZE[1]))
                    results1 = net.predict(segment)
                    overlap_results1 = (0.5 * segment + 0.5 * results1[0])
                    overlap_results1 = overlap_results1/255.0

                    # FPS
                    FPS = (1 / (time.time() - last_time))
                    print("FPS: {}".format(FPS))
                    
                    # Output
                    # cv2.putText(capture, "FPS: {}".format(FPS), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
                    # bottomLeftCornerOfText = (0, 100)
                    # cv2.putText(capture, "Steering Ange: {}".format(degrees), bottomLeftCornerOfText, font, fontScale, fontColor, lineType)
                    cv2.imshow("Screen", capture)
                    cv2.imshow("Steer NN", image)
                    cv2.imshow("ICNet", overlap_results1)

                    # Press "q" to quit
                    if cv2.waitKey(25) & 0xFF == ord("q"):
                        cv2.destroyAllWindows()
                        break